...
class BEVTruncatedBatchSampler(Sampler):
    def __init__(self, index_list, batch_size, cfg, shuffle=False):
        """
        ...
        print(f"bptt seq length is {self.sequence_length_bptt}")
        ...
        self.num_samples = self.num_samples // self.sequence_length_bptt
    ...
    def __iter__(self):
        ...
        indices = indices[: self.num_samples * self.batch_size * self.sequence_length_bptt]
        ...
        for i in range(0, len(indices), self.batch_size * self.sequence_length):
            for j in range(i, i + self.sequence_length, self.sequence_length_bptt):
                yield [
...
class BEVDataModuleTemporalTruncated(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.data_train: Optional[BevDatasetTemporalTruncated] = None
        self.data_val: Optional[BevDatasetTemporalTruncated] = None
        self.data_test: Optional[BevDatasetTemporalTruncated] = None
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.data_train = BevDatasetTemporalTruncated(self.cfg_dataset, mode="train")
        self.data_val = BevDatasetTemporalTruncated(self.cfg_dataset, mode="val")
        print("Created Val and Training dataset")
        if self.cfg_dataset.return_test:
            self.data_test = BevDatasetTemporalTruncated(self.cfg_dataset, mode="test")
        else:
