...
class LightningBEVMultiGNLL(LightningModule):
    def __init__(
        self,
    ...
    def training_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        loss_total = 0
        loss_for_logging = {}
        ###### Separate the Variance of elevation
        pred_ele_variance_logging = {}

        if len(self.range_keys) >= 2:
            ### Adding the consistency loss between the predictions
            loss_consistency_logging = torch.zeros(1).to(
                batch["target"]["micro"][0].device
            )
        ...
        for gridmap_key in self.range_keys:
            ...
            pred_ele_variance_logging[gridmap_key] = torch.zeros(1).to(
                batch["target"][gridmap_key][0].device
            )
        ...
        for seq_i in range(bptt_seq_length):
            pred = self(batch, seq_i, batch_idx)

            target = {}
            aux = {}

            for gridmap_key in self.range_keys:
                target[gridmap_key] = batch["target"][gridmap_key][:, seq_i, ...]
                aux[gridmap_key] = batch["aux"][gridmap_key][:, seq_i, ...]
            ...
            for gridmap_key in self.range_keys:
                pred_ele_variance_logging[gridmap_key] += (
                    final_pred[gridmap_key][:, 2].mean().item()
                )
                final_pred[gridmap_key] = final_pred[gridmap_key][:, :2]

            if len(self.range_keys) >= 2:
                ### Adding the consistency loss between the predictions
                cropped_short = center_crop(
                    final_pred["short"][:, 1][:, None], (128, 128)
                )
                downsampled_micro = F.interpolate(
                    final_pred["micro"][:, 1][:, None],
                    (128, 128),
                    mode="bilinear",
                    align_corners=True,
                )
                loss_consistency = (
                    0.01
                    * F.smooth_l1_loss(
                        cropped_short, downsampled_micro, reduction="none"
                    ).mean()
                )
            ...
            self.visu(batch, final_pred, batch_idx, seq_i)

        for gridmap_key in self.range_keys:
            self.log(
                f"train/loss/{gridmap_key}_variance",
                pred_ele_variance_logging[gridmap_key].item() / bptt_seq_length,
        ...
        if len(self.range_keys) >= 2:
            self.log(
                "train/loss/consistency",
                loss_consistency.item() / bptt_seq_length,
                prog_bar=True,
            )
    ...
    def validation_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        ...
        for gridmap_key in self.range_keys:
            #### Log the variance
            self.log(
                f"val/loss/{gridmap_key}_variance",
                pred_ele_variance_logging[gridmap_key].item() / bptt_seq_length,
                batch_size=self.hparams.batch_size,
            )
    ...
    def test_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        ...
        for gridmap_key in self.range_keys:
            #### Log the variance
            self.log(
                f"test/loss/{gridmap_key}_variance",
                pred_ele_variance_logging[gridmap_key].item() / bptt_seq_length,
                batch_size=self.hparams.batch_size,
            )
    ...
    def get_scaled_layer(self, layer_key, layer_cfg, data, gridmap_key, aux=False):
        if aux:
            idx_dict = self.aux_idxs_dict[gridmap_key]
        else:
            idx_dict = self.target_idxs_dict[gridmap_key]
    ...
    @torch.no_grad()
    @accumulate_time
    def visu(self, batch, pred, batch_idx, seq_i, pred_std=None):
        with torch.no_grad():
            if not (
            ...
            post_rots = batch["post_rots"][:, seq_i, ...]
            post_trans = batch["post_trans"][:, seq_i, ...]
            # target = batch["target"] # Need to give the gridmap key
            # aux = batch["aux"] # Need to give the gridmap key
            img_plots = batch["img_plots"][:, seq_i, ...]
            # gmr = batch["gm_res"] # Need to give the gridmap key
            pcd = batch["pcd"][seq_i]
            gvom = batch["gvom"][seq_i]
            item_idx = batch["index"][:, seq_i, ...]
            BS = imgs.shape[0]

            # Now we need to loop over the range keys
            # FOr now lets use hardcoded ranges and resolution for the PCD and GVOM
            gridmap_shape = {}
            gridmap_resolution = {}
            ...
            for gridmap_key in self.range_keys:
                ...
                if (
                    ...
                    pcd_data = voxelize_pcd_scans(
                        ...
                        gridmap_shape[gridmap_key],
                        gridmap_resolution[gridmap_key],
                    )
                ...
                aux_layers_cfg = self.hparams.metrics.aux_layers[gridmap_key].layers
                target_layers_cfg = self.hparams.metrics.target_layers[
                    gridmap_key
                ].layers
                ...
                for b in range(BS):
                    if (
                        ...
                        if self.hparams.visualizer.tag_by_dataloader:
                            if self._mode == "train":
                                seq_key = self.trainer.train_dataloader.dataset.get_sequence_key(
                                    item_idx[b].item(),
                                    self.hparams.visualizer.tag_with_front_camera,
                                )
                            elif self._mode == "val":
                                seq_key = self.trainer.val_dataloaders.dataset.get_sequence_key(
                                    item_idx[b].item(),
                                    self.hparams.visualizer.tag_with_front_camera,
                                )
                            ...
                            img_idx_str = (
                                seq_key + "_" + gridmap_key + "_" + img_idx_str
                            )
