            ...
            for b in range(BS):
                if (
                    ...
                    if imgs is not None:
                        ...
                        m_confidence_gt = aux[:, confidence_idx] > 0.5
                        m_reliable = aux[:, reliable_idx] > 0.5
                        m_unobserved = m_confidence_gt * ~m_reliable

                        invalid_m = torch.bitwise_or(
                            aux[b, self.hparams.metrics.aux] < 0.5,
                            torch.isnan(
                        ...
                        inpainted_risk[~m_confidence_gt] = torch.nan

                        inpainted_elevation = current_elevation_raw_inpainted[b]
                        inpainted_elevation[~m_confidence_gt] = torch.nan
