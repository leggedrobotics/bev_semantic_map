...
class LightningBEVTemporalTruncated(LightningBEV):
    def __init__(
        self,
    ...
    def forward(self, batch: torch.tensor, seq_i: int, batch_idx: int):
        return self.net(
            ...
            target_shape=batch["target"][:, seq_i, ...].shape,
            pcd=batch["pcd"][seq_i],
    ...
    def training_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        loss_total = 0
        loss_for_logging = torch.zeros(batch["target"][0].shape[1]).to(batch["target"][0].device)

        bptt_seq_length = batch["imgs"].shape[1]
        if batch_idx % self.sequence_length//bptt_seq_length == 0:
            self.net.reset_temporal(batch["aux"][:,0,...], batch["H_sg_map"][:,0,...])
        ...
        for i, layer in enumerate(self.hparams.metrics.target_layers.keys()):
            self.log(
                f"train_{layer}",
                loss_for_logging[i].item() / bptt_seq_length,
                prog_bar=True,
                batch_size=self.hparams.batch_size,
            )

        self.log("train_loss", loss_total.item() / bptt_seq_length, prog_bar=True)
    ...
    def validation_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        ...
        self.log("val_loss", loss_total.item() / bptt_seq_length, prog_bar=True)
    ...
    def test_step(self, batch: any, batch_idx: int) -> torch.Tensor:
        ...
        self.log("test_loss", loss_total.item() / bptt_seq_length, prog_bar=True)
