...
class BevTravNetMulti(nn.Module):
    def __init__(self, cfg_model: Dict[str, Any], **kwargs) -> None:
        ...
        self.minz_rescale = self.cfg_model.elevation_rescale
        self.min_z_scatter = nn.ModuleDict()
        self.use_minz = {}
        self.use_minz["lidar"] = cfg_model.use_minz_pcl
        self.use_minz["gvom"] = cfg_model.use_minz_gvom
        ...
        if cfg_model.use_lidar:
            ...
            if cfg_model.use_minz_pcl:
                self.min_z_scatter["lidar"] = PointPillarsScatter(
                    in_channels=3,
        ...
        if cfg_model.use_gvom:
            ...
            if cfg_model.use_minz_gvom:
                self.min_z_scatter["gvom"] = PointPillarsScatter(
                    in_channels=3,
                    output_shape=encoders["gvom"]["backbone"]["middle_encoder"][
                        "output_shape"
                    ],
                )
                fuser_input_channels.append(1)
        ...
        if cfg_model.use_raw_elevation:
            fuser_input_channels.append(1)
            # We will use the short range raw elevation (Micro only for the micro experiment)
            try:
                self.elevation_idx = [
                    j
                ...
                self.elevation_idx = [
                    j
                    for j, l in enumerate(cfg_model.aux_layers["micro"].layers.values())
                    if (l.name == "elevation_raw")
                ][0]
    ...
    def extract_features(self, x, sensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            if self.use_minz[sensor]:
                # Extract the Min Z for PCL/GVOM
                z_values = feats[:, :, 2]
                ...
                min_z_feat = self.min_z_scatter[sensor](
                    min_z_features, coords, batch_size
                )
                min_z_feat = min_z_feat[:, 2][:, None]
                min_z_feat[min_z_feat >= 5000] = 0
                min_z_feat = min_z_feat * self.minz_rescale
    ...
    def forward(
        ...
        for sensor in self.encoders:
            if sensor == "camera":
                ...
                feature, min_z_feat = self.extract_features(pcd, sensor)
            elif sensor == "gvom":
            ...
            if sensor == "lidar" or sensor == "gvom":
                if self.use_minz[sensor]:
        ...
        if self.cfg_model.use_temporal_fusion:
            ...
            self.bev_features_hidden = self.temporal_fusion(
                torch.cat([fused_output, bev_features_new], dim=1)
            )
            self.prev_H_sg_map = H_sg_map
        else:
