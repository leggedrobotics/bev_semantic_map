...
class BevTravNetTemporalTestEle(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.gmr_resolution = self.cfg_model.gmr_resolution * self.cfg_model.downsample_raw_ele
    ...
    def reset_temporal(self, aux, H_sg_map):
        ...
        self.H_c = H // 2
        self.W_c = W // 2
        # self.bev_features_hidden = torch.nan_to_num(aux[:, self.elevation_idx], 0)[:, None]
        self.bev_features_hidden = aux[:, self.elevation_idx][:, None]
        if self.cfg_model.downsample_raw_ele > 1:
            self.bev_features_hidden = torch.nn.functional.interpolate(self.bev_features_hidden, scale_factor=1.0/self.cfg_model.downsample_raw_ele, mode="bilinear")
    ...
    def compensate_ego_motion(self, H_sg_map, bev_features):
        ...
        for i in range(B):
            ...
            sh = [shift[1], shift[0]]
            yaw = ypr[0]
            print(f"Z offset is {z_offset}")
            bev_features[i][0] = bev_features[i][0] + z_offset
            # print("sh ", i, list(sh))
            # print("yaw ", i, yaw)
    ...
    def forward(
        ...
        if self.cfg_model.use_raw_elevation:
            if self.cfg_model.use_temporal_fusion:
                ...
                ele_features = self.compensate_ego_motion(
                H_sg_map, self.bev_features_hidden.clone().detach()
                )
                ...
                if self.cfg_model.downsample_raw_ele > 1:
                    ele_features = torch.nn.functional.interpolate(ele_features, scale_factor=1.0/self.cfg_model.downsample_raw_ele, mode="bilinear")
                # features.append(ele_features)

            # Compute the error between the two
            ele_feat_current = aux[:, self.elevation_idx][:, None]
            mask = ~(torch.isnan(ele_features)) * ~(torch.isnan(ele_feat_current))
            
            error = F.mse_loss(ele_features[mask], ele_feat_current[mask], reduction="mean")
            print(f"batch idx {batch_idx} error is {20*error}")


            # save the raw elevation
             
            raw_tf_ele = torch.flip(ele_features.detach().cpu()[2], dims=(1,2)) * 20
            raw_curr_ele = torch.flip(ele_feat_current.detach().cpu()[2], dims=(1,2)) * 20
            error_ele = torch.abs(raw_tf_ele - raw_curr_ele)
            maps = torch.stack([raw_tf_ele, raw_curr_ele, error_ele], dim=0,)
            ...
            plt.savefig(f'/work/09654/mhpatel/ls6/results/images_batch/{batch_idx}.png')
            plt.close()

            features.append(ele_feat_current)
        ...
        # self.bev_features_hidden = x["elevation"] # TODO Remove harcoded 1 for elevation index
