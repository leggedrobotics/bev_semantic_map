...
class BaseTransform(nn.Module):
    def __init__(
        ...
        self.feature_size = [
            image_size[0] // feature_downsample,
            image_size[1] // feature_downsample,
        ]
        # self.feature_size = [50, 80]  # TODO
        self.xbound = xbound
        self.ybound = ybound
        self.zbound = zbound
        self.dbound = dbound
        ...
        **kwargs,
        ...
        if self.height_expand:
            for b in range(len(points)):
                ...
                points_repeated[:, 2] = torch.arange(0.25, 2.25, 0.25).repeat(
                    points[b].shape[0]
                )
        ...
        for b in range(batch_size):
            ...
            for c in range(on_img.shape[0]):
                ...
                if self.depth_input == "scalar":
                    depth[
                        b, c, 0, masked_coords[:, 0], masked_coords[:, 1]
                    ] = masked_dist
                ...
                if self.add_depth_features:
                    depth[
                        ...
                        masked_coords[:, 0],
                        masked_coords[:, 1],
                    ] = points[b][boolmask2idx(on_img[c])].transpose(0, 1)
