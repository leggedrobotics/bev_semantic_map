...
__global__ void ball_query_kernel(int b, int n, int m, float min_radius, float max_radius, int nsample,
...
void ball_query_kernel_launcher(int b, int n, int m, float min_radius, float max_radius, int nsample,
                                const float* new_xyz, const float* xyz, int* idx, cudaStream_t stream)
{
  ...
  ball_query_kernel<<<blocks, threads, 0, stream>>>(b, n, m, min_radius, max_radius, nsample, new_xyz, xyz, idx);
