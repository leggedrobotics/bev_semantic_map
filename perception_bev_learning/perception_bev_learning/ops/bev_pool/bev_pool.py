...
torch.ops.load_library(f"{current_script_dir}/bev_pool_ext.so")
...
@torch.jit.script
def QuickCumsumCudaScript(x, geom_feats, ranks, B: int, D: int, H: int, W: int):
    kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)
    kept[1:] = ranks[1:] != ranks[:-1]
    interval_starts = torch.where(kept)[0].int()
    interval_lengths = torch.zeros_like(interval_starts)
    ...
    out = torch.ops.bev_pool_ops.bev_pool_ts(
        x, geom_feats, interval_lengths, interval_starts, B, D, H, W
    )
    return out
...
def bev_pool(feats, coords, B, D, H, W):
    ...
    # Uncomment the line below and comment the line above while exporting torch script model
    # x = QuickCumsumCudaScript(feats, coords, ranks, B, D, H, W)
