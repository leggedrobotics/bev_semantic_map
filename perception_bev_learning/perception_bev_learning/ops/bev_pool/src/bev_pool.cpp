...
/*
  ...
  Return:
    out              : output features, FloatTensor[b, d, h, w, c]
*/
at::Tensor bev_pool_ts(const at::Tensor _x, const at::Tensor _geom_feats, const at::Tensor _interval_lengths,
                       const at::Tensor _interval_starts, int64_t b, int64_t d, int64_t h, int64_t w)
{
  int n = _x.size(0);
  int c = _x.size(1);
  int n_intervals = _interval_lengths.size(0);
...
TORCH_LIBRARY(bev_pool_ops, m)
{
  m.def("bev_pool_ts", bev_pool_ts);
}