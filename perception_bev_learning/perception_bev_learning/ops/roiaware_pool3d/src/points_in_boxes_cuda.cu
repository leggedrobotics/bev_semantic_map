...
__global__ void points_in_boxes_kernel(int batch_size, int boxes_num, int pts_num, const float* boxes, const float* pts,
                                       int* box_idx_of_points)
{
  ...
  if (bs_idx >= batch_size || pt_idx >= pts_num)
    return;
...
__global__ void points_in_boxes_batch_kernel(int batch_size, int boxes_num, int pts_num, const float* boxes,
...
void points_in_boxes_launcher(int batch_size, int boxes_num, int pts_num, const float* boxes, const float* pts,
                              int* box_idx_of_points)
{
  // params boxes: (B, N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is
  // the bottom center, each box DO NOT overlaps params pts: (B, npoints, 3) [x,
...
void points_in_boxes_batch_launcher(int batch_size, int boxes_num, int pts_num, const float* boxes, const float* pts,
                                    int* box_idx_of_points)
{
  // params boxes: (B, N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is
  ...
  dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
  dim3 threads(THREADS_PER_BLOCK);
  points_in_boxes_batch_kernel<<<blocks, threads>>>(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
...
int points_in_boxes_batch(at::Tensor boxes_tensor, at::Tensor pts_tensor, at::Tensor box_idx_of_points_tensor)
{
  ...
  points_in_boxes_batch_launcher(batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points);
