...
public:
  ...
  TV_HOST_DEVICE_INLINE const T* data() const
  {
    return mArray;
  }
  TV_HOST_DEVICE_INLINE size_t empty() const
  {
    return mSize == 0;
  }
  ...
  public:
    ...
    TV_HOST_DEVICE_INLINE reference operator*()
    {
      return *ptr_;
    }
    TV_HOST_DEVICE_INLINE pointer operator->()
    {
      return ptr_;
    }
  ...
  TV_HOST_DEVICE_INLINE iterator begin()
  {
...
struct ShapeBase : public SimpleVector<int, MaxDim>
{
  ...
  TV_HOST_DEVICE_INLINE size_t ndim() const
  {
    return this->mSize;
  }
...
struct TensorView
{
  ...
  TV_HOST_DEVICE_INLINE T& operator()(Inds... inds)
  {
    ...
    TV_REQUIRE(sizeof...(inds) == mShape.ndim(), "you provide %d indexes, but dim is %d\n", sizeof...(inds),
               mShape.ndim());
    for (int i = 0; i < sizeof...(inds); ++i)
    {
      TV_REQUIRE(idxes[i] >= 0 && idxes[i] < mShape[i], "index-%d(%d) out-of-range: [0, %d)\n", i, idxes[i], mShape[i]);
    }
#endif
  ...
  TV_HOST_DEVICE_INLINE TensorView<T, Rank> operator[](SimpleVector<Slice> slice_vec)
  {
  ...
  TV_HOST_DEVICE_INLINE const Shape& shape() const
  {
    return mShape;
  }
  TV_HOST_DEVICE_INLINE int dim(int idx) const
  {
    return mShape[idx];
  }
  TV_HOST_DEVICE_INLINE int ndim() const
  {
    return mShape.ndim();
  }
  template <class... Inds>
...
protected:
  ...
  TV_HOST_DEVICE_INLINE Slice to_slice(Slice s) const
  {
    return Slice(s);
  }
