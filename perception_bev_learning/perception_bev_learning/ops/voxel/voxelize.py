...
torch.ops.load_library(f"{current_script_dir}/voxel_layer.so")
...
def voxelization_script(
    ...
    voxels = points.new_zeros(
        size=(int(max_voxels), int(max_points), int(points.size(1)))
    )
    coors = points.new_zeros(size=(int(max_voxels), 3), dtype=torch.int)
    num_points_per_voxel = points.new_zeros(size=(int(max_voxels),), dtype=torch.int)
    voxel_num = torch.ops.voxel_ops.hard_voxelize_ts(
        points,
        voxels,
        coors,
        num_points_per_voxel,
...
class Voxelization(nn.Module):
    def __init__(
        ...
        self.pcr = torch.tensor(
            point_cloud_range, dtype=torch.float32, requires_grad=False
        )
        # [0, -40, -3, 70.4, 40, 1]
        voxel_size = torch.tensor(voxel_size, dtype=torch.float32)
        self.vs = torch.tensor(voxel_size, dtype=torch.float32, requires_grad=False)
    ...
    def forward(self, input):
        ...
        #     self.deterministic,
        # )
        # Uncomment the line above and comment the line below while exporting torch script model
        return voxelization(
            input,
            self.voxel_size,
