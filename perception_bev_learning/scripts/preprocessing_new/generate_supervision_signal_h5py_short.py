...
class GenerateGTSupervision:
    ...
    def compute(self, h5py_file, pkl_file):
        ...
        for gridmap_key, fusions in self.gridmap_fusion_dict.items():
            print(f"Processing gridmap key {gridmap_key}")

            gridmap_size = self.gridmap_cfg_dict[gridmap_key].HW_ext
            gridmap_gt_key = self.obs_key_to_gt_key[gridmap_key]
            max_acc_time = self.gridmap_cfg_dict[gridmap_key].max_acc_time
            reliable_threshold = self.gridmap_cfg_dict[gridmap_key].reliable_threshold
            # reliable_layer = self.gridmap_cfg_dict[gridmap_key].reliable_layer
                ...
                with tqdm(
                    ...
                    for idx in idx_mapping:
                        ...
                        for k in possible_idx[m][::subdiv]:
                            ...
                            # reliable = data_shifted[layer_idx_dict["reliable"]]
                            # reliable[np.isnan(reliable)] = 0

                            confidence = data_shifted[layer_idx_dict["confidence"]]
                            ...
                            # elevation_est = data_shifted[layer_idx_dict["elevation"]]

                            # data_shifted = np.concatenate(
                            #     [data_shifted, robust_reliable[None]], axis=0
                            # )
                            # data_shifted = np.concatenate(
                            #     [data_shifted, elevation_est[None]], axis=0
                            # )

                            # Select the reliable layer
                            # m = robust_reliable < reliable_threshold
                            m_wheel_risk = confidence < 0.1
                            m_elevation = confidence < 0.8
                            m_elevation = np.array(m_elevation, dtype=np.float32)
                            m_elevation = cv2.dilate(m_elevation, kernel)
                            m_elevation = m_elevation > 0

                            for layer in self.gridmap_cfg_dict[gridmap_key].risk_layer:
                                data_shifted[layer_idx_dict[layer]][
                                    m_wheel_risk
                                ] = np.nan

                            # data_shifted[layer_idx_dict["elevation"]][m_elevation] = np.nan

                            for j, l in enumerate(layers):
                                map1[j], map2[j] = fusions[l].fuse(
                                    data_shifted[j], map1[j], map2[j], confidence
                                )
