                        ...
                        if in_key == self.cfg.dem_layer_key:
                            try:
                                ...
                                pointcloud2 = gridmap_to_pointcloud(
                                    gridmap2, multiplier=1
                                )
                                pointcloud_short = gridmap_to_pointcloud(
                                    gridmap_short, multiplier=1
                                )
                                ...
                                gridmap2_new = np.array(
                                    gridmap2_new.squeeze(0).squeeze(0)
                                )
                                ...
                                gridmap_short_new = center_crop(
                                    gridmap_short_new,
                                    output_size=self.dem_cfg.crop_gridmap_short,
                                )
                                ...
                                # gridmap_duplicate[dilated_fusion_edges] = smoothed_img[dilated_fusion_edges]

                                # gridmap2_new[conf_idxs] = gridmap1[conf_idxs]
                                ...
                                #####################################
                                # Saving the gridmap short data
                                static = {
                                    "layers": ["elevation"],
                                    "resolution": np.array([1.0]),
                                    "length": np.array((1 * 200, 1 * 200)).reshape(-1),
                                    "header_frame_id": "sensor_gravity",
                                }
                                dynamic = {
                                    "data": gridmap_short_new,
                                ...
                                if self.write:
                                    dataset_writer.add_static(
                                        sequence=seq_name,
                                        fieldname=self.dem_cfg.output_short_key,
                                        data_dict=static,
                                    )
                            ...
                            #####################################
                                ...
                                grid_map_data_rotated = np.concatenate(
                                    (
                                        grid_map_data_rotated,
                                        np.expand_dims(
                                            np.zeros_like(grid_map_data_rotated[0]),
                                            axis=0,
                                        ),
                                    )
                                )
