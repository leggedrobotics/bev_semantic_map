...
from visualize_gt_short_color import SimpleNumpyToRviz
from visualize_gt_short_color import get_H_h5py, inv, get_gravity_aligned
...
class GravityAlignedGridMaps:
    def __init__(self, cfg):
        ...
        self.layer_dict = self.cfg.layer_keys
        self.trans_threshold = self.cfg.trans_threshold
        self.rot_threshold = self.cfg.rot_threshold
    ...
    def compute(self, h5py_file, pkl_file):
        ...
        for in_key in self.layer_dict.keys():
            out_key = self.layer_dict[in_key].output_key
        ...
        for in_key in self.layer_dict.keys():
            ...
            if in_key == self.cfg.dem_layer_key:
                for keys, vals in self.dem_cfg.output_keys.items():
                    output_dem_key = vals["output_key"]

                    if output_dem_key not in headers:
                        self.pd_df[output_dem_key] = self.pd_df[self.cfg.anchor_key]
                    try:
                        del h5py_seq[output_dem_key]
                    except:
                        print(f"GT key {output_dem_key} does not exist")
                        pass

                # Add the boolean Column for ICP results
                if "icp_threshold" not in headers:
                    self.pd_df["icp_threshold"] = True

                print(self.pd_df)

                # We need to do DEM processing and fusion

                # elevation_layer_key_current = in_key - "_gt"
                # h5py_gridmap_current = h5py_seq[elevation_layer_key_current]
                # layers_current = [l.decode("utf-8") for l in h5py_gridmap_current["layers"]]
                # ele_idx_current = layers_current.index(self.dem_cfg.elevation_layer)
            ...
            H_ext, W_ext = self.layer_dict[in_key].HW
            res_cfg = self.layer_dict[in_key].resolution

            upsample_factor = res[0] / res_cfg

            if upsample_factor < 1.01 and upsample_factor > 0.99:
                upsample_factor = 1

            print(f"Upsample factor is {upsample_factor}")

            pad_val = [
                int((H_ext - H * upsample_factor) / 2),
                int((W_ext - W * upsample_factor) / 2),
            ]
            ...
            try:
                with tqdm(
                    ...
                    for idx in idx_mapping:
                        ...
                        grid_map_data_rotated = F.interpolate(
                            grid_map_data_rotated,
                            scale_factor=upsample_factor,
                            mode="bilinear",
                            align_corners=True,
                        )
                        grid_map_data_rotated = grid_map_data_rotated[0]
                        ...
                        if pad_val[0] > 0:
                            ...
                            grid_map_data_rotated = center_crop(
                                grid_map_data_rotated, (H_ext, W_ext)
                            )
                        ...
                        #         grid_map_data_sat_micro = center_crop(
                        #             grid_map_data_sat_rotated,
                        #             self.dem_cfg.crop_gridmap_micro,
                        #         )
                        #         grid_map_data_sat_short = center_crop(
                        #             grid_map_data_sat_rotated,
                        #             self.dem_cfg.crop_gridmap_short,
                        #         )

                        #         elevation_values_micro = np.array(
                        #             grid_map_data_sat_micro[:, :]
                        #         )
                        #         elevation_values_short = np.array(
                        #             grid_map_data_sat_short[:, :]
                        #         )

                        #         elevation_gt = grid_map_data_rotated[
                        #             ele_idx_dem
                        #         ]  # From current gridmap
                        #         confidence_gt = grid_map_data_rotated[
                        #             conf_idx_dem
                        #         ]  # From current gridmap

                        #         ele_veh_gt = (
                        #             -1.5
                        #         )  # Rough location of sensor origin link w.r.t Ground
                        #         ele_veh_sat = elevation_values_micro[
                        #             elevation_values_micro.shape[0] // 2,
                        #             elevation_values_micro.shape[1] // 2,
                        #         ]  # Take the elevation at the center of the satellite Map which is car location

                        #         elevation_values_micro = elevation_values_micro - (
                        ...
                        #         # Repeat the same process for Short range Map
                        #         elevation_values_short = elevation_values_short - (
                        #             ele_veh_sat - ele_veh_gt
                        #         )
                        #         elevation_values_short = elevation_values_short[
                        #             ::-1, ::-1
                        ...
                        #         gridmap2 = elevation_values_micro
                        #         gridmap_short = elevation_values_short
                        #         ######## ICP #####################
                        #         pointcloud1 = gridmap_to_pointcloud(
                        #             gridmap1, multiplier=res_cfg
                        #         )
                        #         pointcloud2 = gridmap_to_pointcloud(
                        #             gridmap2, multiplier=self.dem_cfg.dem_res
                        #         )
                        #         pointcloud_short = gridmap_to_pointcloud(
                        #             gridmap_short, multiplier=self.dem_cfg.dem_res
                        #         )
                        ...
                        #         # Check if the translations and rotations are less than threshold
                        #         # Keep a track of this in the pickle file

                        #         if (
                        #             result_ypr[0] > self.rot_threshold
                        #             or result_ypr[1] > self.rot_threshold
                        #             or result_ypr[2] > self.rot_threshold
                        #         ):
                        #             self.pd_df.at[idx, "icp_threshold"] = False
                        #             print(
                        #                 f"RPY (degrees) is {result_ypr} \n Translation is {result_tf[:3,3]}"
                        #             )
                        #         if (
                        #             result_tf[0, 3] > self.trans_threshold
                        #             or result_tf[1, 3] > self.trans_threshold
                        #         ):
                        #             self.pd_df.at[idx, "icp_threshold"] = False
                        #             print(
                        #                 f"RPY (degrees) is {result_ypr} \n Translation is {result_tf[:3,3]}"
                        ...
                        #         # draw_registration_result(pointcloud_short, target_down, result_ransac.transformation)

                        #         pointcloud2 = pointcloud2.transform(
                        #             result_ransac.transformation
                        #         )
                        #         gridmap_micro_new = pointcloud_to_gridmap(
                        #             pointcloud2,
                        #             multiplier=self.dem_cfg.dem_res,
                        #             grid_shape=self.dem_cfg.crop_gridmap_tolerance,
                        #         )
                        #         gridmap_micro_new = F.interpolate(
                        #             torch.from_numpy(gridmap_micro_new)
                        #             .unsqueeze(0)
                        #             .unsqueeze(0),
                        #             scale_factor=(
                        #                 self.dem_cfg.dem_res
                        #                 / self.dem_cfg.output_keys.micro.resolution
                        #             ),
                        #             mode="bilinear",
                        #             align_corners=True,
                        #         )
                        #         gridmap_micro_new = center_crop(
                        #             gridmap_micro_new,
                        #             output_size=self.dem_cfg.output_keys.micro.HW,
                        #         )
                        #         gridmap_micro_new = np.array(
                        #             gridmap_micro_new.squeeze(0).squeeze(0)
                        #         )

                        #         pointcloud_short = pointcloud_short.transform(
                        #             result_ransac.transformation
                        #         )
                        #         gridmap_short_new = pointcloud_to_gridmap(
                        #             pointcloud_short,
                        #             multiplier=self.dem_cfg.dem_res,
                        #             grid_shape=self.dem_cfg.crop_gridmap_short_tolerance,
                        #         )
                        #         gridmap_short_new = F.interpolate(
                        #             torch.from_numpy(gridmap_short_new)
                        #             .unsqueeze(0)
                        #             .unsqueeze(0),
                        #             scale_factor=(
                        #                 self.dem_cfg.dem_res
                        #                 / self.dem_cfg.output_keys.short.resolution
                        #             ),
                        #             mode="bilinear",
                        #             align_corners=True,
                        #         )
                        #         gridmap_short_new = center_crop(
                        #             gridmap_short_new,
                        #             output_size=self.dem_cfg.output_keys.short.HW,
                        #         )
                        #         gridmap_short_new = np.array(
                        #             gridmap_short_new.squeeze(0)
                        #         )
                        ...
                        #         gridmap_duplicate[conf_idxs] = gridmap_micro_new[
                        #             conf_idxs
                        #         ]
                        #         gridmap_micro_fused = np.expand_dims(
                        #             gridmap_duplicate, axis=0
                        #         )
                        #         gridmap_micro_new = np.expand_dims(
                        #             gridmap_micro_new, axis=0
                        #         )

                        #         #####################################
                        #         for key, val in self.dem_cfg.output_keys.items():
                        #             # Saving the gridmap short and micro data
                        #             if key == "short":
                        #                 gridmap_data = gridmap_short_new
                        #             if key == "micro":
                        #                 gridmap_data = np.concatenate(
                        #                     (gridmap_micro_fused, gridmap_micro_new)
                        #                 )

                        #             static = {
                        #                 "layers": (
                        #                     OmegaConf.to_container(val["layers"])
                        #                 ),
                        #                 "resolution": np.array([val["resolution"]]),
                        #                 "length": np.array(
                        #                     (
                        #                         val["resolution"] * val["HW"][0],
                        #                         val["resolution"] * val["HW"][1],
                        #                     )
                        #                 ).reshape(-1),
                        ...
                        #             dynamic = {
                        #                 "data": gridmap_data,
                        #                 "header_seq": h5py_gridmap["header_seq"][
                        #                     gm_idx
                        #                 ],
                        #                 "header_stamp_nsecs": h5py_gridmap[
                        #                     "header_stamp_nsecs"
                        #                 ][gm_idx],
                        #                 "header_stamp_secs": h5py_gridmap[
                        #                     "header_stamp_secs"
                        #                 ][gm_idx],
                        #                 "orientation_xyzw": h5py_gridmap[
                        #                     "orientation_xyzw"
                        #                 ][gm_idx],
                        #                 "position": h5py_gridmap["position"][gm_idx],
                        #                 "tf_rotation_xyzw": h5py_gridmap[
                        #                     "tf_rotation_xyzw"
                        #                 ][gm_idx],
                        #                 "tf_translation": h5py_gridmap[
                        #                     "tf_translation"
                        #                 ][gm_idx],
                        #                 "T_sensor_gravity__map": np.array(
                        ...
                        static = {
                            "layers": layers,
                            "resolution": np.array([res_cfg]),
                            "length": np.array(
                                (res_cfg * H_ext, res_cfg * W_ext)
                            ).reshape(-1),
                            "header_frame_id": "sensor_gravity",
                        }
                        dynamic = {
                        ...
                        if self.write:
                            dataset_writer.add_static(
                                sequence=seq_name, fieldname=out_key, data_dict=static
                            )
                            dataset_writer.add_data(
                                sequence=seq_name, fieldname=out_key, data_dict=dynamic
                            )
