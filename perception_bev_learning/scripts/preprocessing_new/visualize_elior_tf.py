...
class GTVisualization:
    ...
    def get_depth_data(self, datum, global_idx):
        """
        ...
        with Timer("images"):
            for img_key in self.image_keys:
                ...
                anchor_image = self.h5py_file[sk][self.anchor_key]
                idx = datum[img_key]
                img_arr = np.array(h5py_image[f"image"][idx])
                ...
                print(h5py_image.keys())
                points_map = []
                H_array = []
                # Here we will loop through all the pointcloud data indices and transform them to the Map frame
                with Timer("extract pcl data"):
                    for idx_sus in range(
                        datum[self.anchor_key], datum[self.anchor_key] + 200
                    ):
                        H_base_link__sus = get_np_T(
                            t=anchor_image[
                                f"tf_translation_base_link__wheel_front_left_suspension"
                            ][idx_sus],
                            q=anchor_image[
                                f"tf_rotation_xyzw_base_link__wheel_front_left_suspension"
                            ][idx_sus],
                        )
                        H_map__baselink = get_np_T(
                            t=anchor_image[f"tf_translation_map__base_link"][idx_sus],
                            q=anchor_image[f"tf_rotation_xyzw_map__base_link"][idx_sus],
                        )

                        H_map__sus = H_map__baselink @ H_base_link__sus

                        point_map = H_map__sus[:, 3]

                        points_map.append(point_map)

                points_map = np.vstack(points_map)
                ...
                # image[points_2d[:, 1], points_2d[:, 0]] = [255, 0, 0]

                # overlay = cv2.addWeighted(img_arr, 0.5, image, 0.5, 0)
                circle_color = (0, 0, 255)
                # img_arr[points_2d[:, 1], points_2d[:, 0]] = [0, 0, 255]
                for point in points_2d:
                    x, y = point
                    # Draw a filled circle with a radius of 5 pixels
                    cv2.circle(img_arr, (x, y), 5, circle_color, -1)

                img_dict[img_key] = np.array(img_arr)
    ...
    def get_item(self, idx):
        ...
        image_data = self.get_depth_data(datum, idx)
